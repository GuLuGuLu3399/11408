...
> **只有一种标准解法**（扫描 + 找前驱删除）。遍历的每步中，若 `p` 被删则 `pre` 不动（因为 `pre->next` 已重连到下一个），否则 `pre` 跟到 `p` 原位再前移。此解法对"连续多个 x"天然正确（连续删时 pre 停在原地，逐个剪掉）。
...
**对比**：
- 头插法：思路直观，每步"摘一个→前插"，**你已掌握**。
- 三指针法：pre/p/r 散步式改向，更贴近"数组逆置"的直觉，**更容易泛化到双链表/部分逆置**。两法都要会，复试可能考你不许用头插的情况。
