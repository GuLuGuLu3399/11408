...
```mermaid
flowchart TD
    Q1["遇到一条指令，问："] --> Q2{"它是算数/赋值/数据传送/跳转？"}
    Q2 -- "是" --> USER1["✅ 非特权指令（用户态执行）"]
    Q2 -- "否" --> Q3{"它是 Trap / 访管指令 / Syscall？"}
    Q3 -- "是" --> USER2["✅ 非特权指令<br/>但执行后触发内部中断 → 切换到内核态"]
    Q3 -- "否" --> Q4{"它直接操作以下五类之一？<br/>① I/O 设备 ② 页表/TLB ③ 中断开关<br/>④ PSW/系统时钟 ⑤ 进程调度"}
    Q4 -- "是" --> PRIV["❌ 绝对特权指令（内核态执行）"]
    Q4 -- "否" --> EDGE["⚠️ 边界案例，<br/>套用上方对比表判断"]
```
...
| 象限              |   执行地   |   处理地   | 典型代表                | 说明                                                               |
| :---------------- | :--------: | :--------: | :---------------------- | :----------------------------------------------------------------- |
| **Ⅰ 普通用户区**  |   用户态   |   用户态   | 算术运算、MOV、JMP      | 最安全的指令，完全在用户空间完成                                   |
| **Ⅱ ❌ 不可能区** |   内核态   |   用户态   | —                       | 进了内核态就不可能把控制权交给用户来处理                           |
| **Ⅲ 混合触发区**  | **用户态** | **内核态** | Trap、缺页、除零        | ⚠️ **最容易混淆的陷阱区！** 事件发生在用户态，但 OS 在内核态擦屁股 |
| **Ⅳ 内核管理区**  |   内核态   |   内核态   | 修改页表、CLI、进程切换 | 完全在内核域完成，用户不可见                                       |
...
| 系统类型       | 首要追求目标           | CPU 调度方式           |   交互性    | 典型应用场景           | 一句话记忆       |
| :------------- | :--------------------- | :--------------------- | :---------: | :--------------------- | :--------------- |
| **单道批处理** | 自动平滑过渡           | 顺序串行               |    ❌ 无    | 极早期电子管计算机     | "串行等，CPU 闲" |
| **多道批处理** | **吞吐量、资源利用率** | 无抢占并发（I/O 才切） |    ❌ 无    | 大型科学计算、数据跑批 | "并发跑，交互无" |
| **分时系统**   | **人机交互、响应时间** | **时间片轮转**         | ✅ **极强** | Unix/Linux、日常 PC    | "时间片，人人有" |
| **实时系统**   | **截止时间、可靠性**   | 绝对值优先级抢占       |    ⚠️ 弱    | 武器控制、工业自动化   | "死线到，必须完" |
